...
Promise.all([
	...
	fetch(randomUsersURL),
	fetch(randomUsersURL)
]) //i like this, very terse
	.then(response => Promise.all(response.map(result => result.json())))
	.then(renderUserCard)
	.catch(console.log) //nice use of a catch here! very responsible.

function renderUserCard(userArr) {
	let newCards = userArr
		.map((user, idx) => {
			//Adjust email font size to fit card //// good comment
			let fontSize = ""
	...
	window.location.hash = "#all" //forces hash refresh on reload ////neat!
}

window.addEventListener("hashchange", () => {
	const idStr = window.location.hash.slice(1)
	const idNum = window.location.hash.slice(1) * 1 // "const idNum = idStr * 1" would be a bit less verbose
	...
	if (Number.isNaN(idNum) || idNum >= userCards.length || idNum < 0) {
		userCards.forEach(card => {
			...
			card.style.order = "0"
		}) //again, very responsible stuff here!!
	} else {
